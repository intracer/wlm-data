...
def test_run_monuments_pipeline_writes_all_layers(spark, tmp_path):
    paths = MonumentPaths(
        monuments_csv=str(REPO_ROOT / "data/wiki/monuments/wlm-ua-monuments.csv"),
        humdata_csv=str(REPO_ROOT / "data/humdata/ukraine-populated-places.csv"),
    ...
    for layer_dir in [
        ...
        parquet_files = glob.glob(f"{layer_dir}/*.parquet")
